...
const getCSS = ({ spacing, justify, hyphenate }) => `
    ...
    html {
        color-scheme: ${dark_theme ? "dark" : "light"};
    }
...
class Reader {
  ...
  #onRelocate({ detail }) {
    ...
    if (this.loaded) saveProgress(detail.cfi, Math.floor(fraction * 100));
  }
}

const open = async (file) => {
  console.log(file);
  $("#drop-target").classList.add("hidden");
...
$("#file-input").onchange = () =>
  open($("#file-input").files[0]).catch((e) => console.error(e));

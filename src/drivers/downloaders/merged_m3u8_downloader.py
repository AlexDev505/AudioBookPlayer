...
class MergedM3U8Downloader(BaseAudioDownloader):
    """
    ...
    def _prepare_files_data(self):
        return [
            File(
                index=i,
                name=f"seq{i}.ts",
                url=urljoin(ty.cast(str, self._host_uri), segment.uri),
                extra=dict(duration=getattr(segment, "duration", None)),
            )
            for i, segment in enumerate(
                ty.cast(m3u8.M3U8, self._m3u8_data).segments
            )
        ]

    async def _prepare(self):
        # loads m3u8 file
        self._m3u8_data = m3u8.load(self._book.source.chapters[0].url)
    ...
    def _decrypt_seq(self, file: File) -> None:
        segment = ty.cast(m3u8.M3U8, self._m3u8_data).segments[file.index]
        if decrypt_func := self._get_decryption_func(file.index, segment):
            file_path = os.path.join(self._book.dir_path, file.name)
    ...
    async def _seq_downloaded(self, file: File) -> None:
        ...
        if self._chapter_index + 1 == len(self._book.source.chapters):
        ...
        chapter = self._book.source.chapters[self._chapter_index]
        ts_duration = file.extra["duration"] or get_audio_file_duration(ts_path)
        self._current_duration += ts_duration
        if chapter.duration - self._current_duration < 0.5:
            second = second_time = 0
            split_time = round(
                chapter.duration - (self._current_duration - ts_duration)
            )
    ...
    def _merge_ts_files(
        self, chapter_index: int, ts_file_paths: list[Path]
    ) -> None:
        item_file_name = self._get_chapter_file_name(chapter_index, "")
        logger.opt(colors=True).debug(
            f"merging <y>{len(ts_file_paths)}</y> files "
            f"(<y>{ts_file_paths[0].name}-{ts_file_paths[-1].name}</y>) "
            f"to <y>{item_file_name}.mp3</y>"
        )
        merge_ts_files(ts_file_paths, Path(self._book.dir_path), item_file_name)
        logger.opt(colors=True).debug(
        ...
        self._real_item_paths.append(
            Path(self._book.dir_path, item_file_name + ".mp3")
        )
    ...
    def _parse_host_uri(self) -> None:
        host_uri = urlparse(self._book.source.chapters[0].url)
        self._host_uri = (

...
function toggleDriver(driver, btn) {
  ...
  if ((required_drivers.length == 1) & required_drivers.includes(driver))
    return;
  ...
  if (document.querySelector("#search-input-line input").value.trim())
    searchBooks();
}
...
async function searchBooks() {
  ...
  pywebview.api
    .search_books(query, (required_drivers = required_drivers))
...
function onSearchCompleted(resp, clear = true) {
  ...
  for (book of resp) {
    ...
    html =
      html +
      `<div class="search-result-item" data-url="${book.url}">
  ...
  if (!resp.length && clear)
    html = '<div id="no-search-result">{{ gettext("search.empty") }}</div>';
  container = document.getElementById("search-results-container");
  ...
  if (clear) {
    ...
    if (resp.length == 0) can_search_next = false;
  }
...
function addBook(el) {
  ...
  pywebview.api
    .add_book_to_library(el.parentElement.dataset.url)
    .then((response) => {
      el.classList.remove("loading");
      if (response.status != "ok") {
        showError(response.message);
      } else el.classList.add("added");
    });
}

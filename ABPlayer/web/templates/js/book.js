...
function loadBookData(bid) {
    pywebview.api.book_by_bid(bid, true).then((resp) => {
        ...
        document.querySelector("#book-page-content .book-listening-progress").innerHTML = `${resp.data.listening_progress} {{ gettext("book.listening_progress")}}`
        document.querySelector("#book-page-content .book-adding-date").innerHTML = `{{ gettext("book.added") }} ${resp.data.adding_date}`
